...
const App: React.FC = () => {
    ...
    // Fetch real AQI data from backend - ONLY ONCE on mount
    const aqiLoadedRef = React.useRef(false);

    useEffect(() => {
        // Only fetch once on mount
        if (aqiLoadedRef.current) return;
        aqiLoadedRef.current = true;

        const fetchAQI = async () => {
            try {
                setAqiLoading(true);
                const response = await fetch('https://musaffo-iqair-service-242593050011.us-central1.run.app/api/air-quality?city=Tashkent&country=Uzbekistan');
